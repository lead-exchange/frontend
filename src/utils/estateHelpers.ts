...
export const getAddressParts = (address: RealEstateObject['attributes']['address']): string[] => {
  const parts = [
    address.regionName && address.regionType ? `${address.regionName} ${address.regionType}` : address.regionName,
...
export const getEstateChipValues = (attributes: RealEstateObject['attributes']): string[] => {
  ...
  if (attributes.realtyType) chipValues.push(propertyTypeLabels[attributes.realtyType]);
